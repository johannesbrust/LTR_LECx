...
% full path MATLAB
%MATLAB = '/usr/local/MATLAB' ; % 01/31/19, J.B
MATLAB = '/usr/local/MATLAB/R2014a' ; % 01/31/19, J.B
...
%LIBS = [ '-L' PREFIX '/lib -L/usr/lib/gcc/x86_64-linux-gnu/4.8 -lgfortran -lquadmath -lstdc++ -ldl -lm -lc' ] ;
...
%LIBS2 = ['-Wl,rpath-link,' MATLAB '/bin/glnxa64 -L/opt/local2/lib -lipopt -lcoinmumps -lcoinlapack -lcoinblas'];
%LIBS2 = ['-Wl,-Bdynamic -L/opt/local2/lib -lipopt -lcoinmumps -lcoinlapack -lcoinblas '];
...
MEXFLAGS = [ '-v -cxx -largeArrayDims ' ...
             ...
             'CXXOPTIMFLAGS="' CXXFLAGS '" ' ...
             'LDOPTIMFLAGS="" ' ...
             'CXXLIBS=''$CXXLIBS ' [LIBS2,LIBS] ''''  ] ; % -static -shared-libgcc          
...
%              'CXXLIBS=' ['"','$CXXLIBS ',LIBS2,LIBS,' "'] ] ; % -static -shared-libgcc         
         %"$CXXLIBS ' LIBS2 + LIBS ' "' ''
%strcat('"','$CXXLIBS ',LIBS2,LIBS,' "')
% build and execute compilation command
%cmd = sprintf('mex %s %s %s %s',INCL,files,MEXFLAGS,LIBS) ;
cmd = sprintf('mex %s %s %s',INCL,files,MEXFLAGS) ;
%cmd = sprintf('mex %s %s %s %s',INCL,files,MEXFLAGS) ;
eval(cmd);
