...
%}
% 10/15/20, J.B., additional convergence criterion for comparisons
...
switch whichConv
    ...
    case 2
        gpc = -d_;
        convtest = ((norm(gpc,inf) < gtol) && (nb < btol));
    otherwise
        convtest = ((conv<gtol) && (nb < btol));
end

if convtest == 1 %((conv<gtol) && (nb < btol)) % ng<max(1,norm(x0))*gtol,
    ex=1;
    x=x0;
    f=f0;
    tcpu=toc(t0);   
...
% 04/24/20, J.B., modification with inf. norm initialization
nst_inf_2 = ns;
